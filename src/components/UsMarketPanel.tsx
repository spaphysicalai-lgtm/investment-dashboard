...
export default function UsMarketPanel() {
  const [marketData, setMarketData] = useState<UsMarketOverview | null>(null);
  ...
  const fetchMarketData = async () => {
    try {
      ...
      const data: UsMarketOverview = await response.json();
      setMarketData(data);
      ...
      setError(err instanceof Error ? err.message : 'Failed to load US market data');
    } finally {
      setLoading(false);
    }
  };
  ...
  if (loading && !marketData) {
    return (
      <div className="bg-white rounded-lg shadow-lg p-6">
        <h2 className="text-2xl font-bold mb-6 text-gray-800">🇺🇸 US Market</h2>
  ...
  return (
    <div className="bg-white rounded-lg shadow-lg p-6">
      ...
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold text-gray-800">🇺🇸 US Market</h2>
        <button
      ...
      <div className="mb-6 bg-blue-50 rounded-lg p-4">
        ...
        <div className="flex items-baseline gap-3">
          <span className="text-2xl font-bold text-gray-900">
            {marketData.index.value.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
          </span>
        ...
        <table className="w-full">
          <thead>
            <tr className="border-b-2 border-gray-200">
              <th className="py-2 text-left text-xs font-semibold text-gray-600">Symbol</th>
              <th className="py-2 text-left text-xs font-semibold text-gray-600">Name</th>
              <th className="py-2 text-right text-xs font-semibold text-gray-600">Price</th>
              <th className="py-2 text-right text-xs font-semibold text-gray-600">Change</th>
            </tr>
          </thead>
          <tbody>
            {marketData.topGainers.map((stock) => (
              <tr key={stock.symbol} className="border-b border-gray-100 hover:bg-gray-50">
                ...
                <td className="py-3 text-sm text-right font-semibold">${stock.price.toFixed(2)}</td>
